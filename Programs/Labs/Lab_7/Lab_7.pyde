...
#-----------KEY 2: TASK 2--------------------------------
def fractionTree(n, num=1,denom=2):
    T2 = Tree()
    if n != 0:
        str_num = str(num) + '/' + str(denom)
        T2.attach(T2.add_root(str_num), fractionTree(n-1, num*2-1, denom=denom*2), fractionTree(n-1, num*2+1, denom=denom*2))
    return T2
def key2():
    global T
    T=fractionTree(5)
