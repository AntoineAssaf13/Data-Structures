class ExpressionTree:
  ...
  def _unattach(self, p):
      T1 = ExpressionTree()
      T2 = ExpressionTree()
      p1 = self.left(p)
      p2 = self.right(p)
      p1._node._parent = None
      p2._node._parent = None
      p._node._left = None
      p._node._right = None
      T1._root = p1._node
      T2._root = p2._node
  ...
  def hasx(self, p):
      lst = []
      for pos in self._subtree_inorder(p):
          lst.append(pos)
      for ele in lst:
          if ele.element() == 'x':
              return True
...
def solve (string):
    T=build_expression_tree(string)
    p = T.root()
    while T.left(p).element() != 'x' or T.right(p).element() !='x':
        if T.left(p) == None or T.right(p) == None:
            p= T.sibling(p)
        temp1, temp2 = T.unattach(p)
        if temp1.root().element() == '*':
            temp1.replace(temp1.root(), '/')
        if temp1.root().element() == '+':
            temp1.replace(temp1.root(), '-')
        if temp1.root().element() == '-':
            temp1.replace(temp1.root(), '+')
        if temp1.root().element() == '/':
            temp1.replace(temp1.root(), '*')
        T.attach(T.left(p),temp2, temp1)
        p = T.left(p)
    
if __name__ == '__main__':
  tokens = tokenize('((((x+1)*3)/((9-5)+2))-((3*(7-4))+6))')
  print(tokens)
  big = build_expression_tree(tokens)
  big.preorder_indent()
  print(big.hasx(big.root()))
  #print(big, '=', big.evaluate())
  l,r=big._unattach(big.root())
  big.preorder_indent()
  print('left')
  l.preorder_indent()
  print('right')
